...
class BasicBlock(nn.Module):
    ...
    def __init__(self, in_planes, planes, honey, index, stride = 1):
        ...
        self.conv1 = nn.Conv2d(in_planes, int(planes * (1-honey[index] / 10)), kernel_size = 3, stride = stride,
                               padding = 1, bias = False)
        self.bn1 = nn.BatchNorm2d(int(planes *(1-honey[index] / 10)))
        self.conv2 = nn.Conv2d(int(planes * (1-honey[index] / 10)), planes, kernel_size = 3, stride = 1, padding = 1,
                               bias = False)
...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_planes, planes, honey, index, stride = 1):
        ...
        pr_channels = int(planes * (1-honey[index] / 10))
